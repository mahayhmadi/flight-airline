...
# Cloud-ready: no absolute paths, robust CSV loader, ML fallback, explain mode, white airplane SVG
...
# airplane SVG (white, crisp)
AIRPLANE_SVG = """
<svg width="108" height="108" viewBox="0 0 108 108" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Airplane">
  <defs>
  ...
  <path d="M98 50c0 3-2.4 5.4-5.4 5.4H69L45 82l-9-9 13.5-12.5H29l-9 7-6-6 9.5-12.5h30L69 37h23.6c3 0 5.4 2.4 5.4 5.4V50z"
        fill="url(#g1)" stroke="#FFFFFF" stroke-opacity=".25" stroke-width="1.3" />
  <circle cx="83" cy="32" r="3.5" fill="#FFFFFF" />
</svg>
"""
...
# CSV sources (Cloud-friendly: upload / URL / repo file)
DEFAULT_CSV_REL = "final_optimized_flights.csv"      # rule-based data file (include in repo)
ML_CSV_REL      = "final_optimized_flights_ML.csv"   # ML-optimized data file (optional)
...
csv_path_hint = st.sidebar.text_input(
    "Or local relative path (in repo)",
    value=preferred_rel,
...
def load_csv_resilient(engine_choice, uploaded, url, hint_rel, rb_rel, ml_rel):
    ...
    # 3) typed relative path (repo file)
...
if df is None:
    ...
    st.info(
        ...
        "- **Easiest**: add `final_optimized_flights.csv` to the repo root (next to `app.py`).\n"
        "- Or use **Upload CSV** / **URL** in the sidebar.\n\n"
        f"Tip: You selected **{engine_selected}**. Preferred file would be "
...
with st.sidebar.expander("Glossary — simple definitions", expanded=False):
    st.markdown(
        ...
        "- **Optimized revenue**: expected revenue after applying the recommended price.\n"
...
if EXPLAIN_MODE:
    st.info(
        "How to read: KPIs show current vs optimized revenue and uplift. "
        "Table lists price recommendations; charts summarize where improvements come from.",
        icon="ℹ️"
    )
st.markdown("")
...
if engine_used=="ML-based" and not ml_models:
    st.warning("ML models not found. What-If uses rule-based expectations (add 'models_seats_ml/' to repo or upload).")
...
# =========================
st.markdown('<div class="hr-soft"></div>', unsafe_allow_html=True)
st.caption("© Dynamic Pricing Simulator — Streamlit + Plotly | Cloud-friendly loader with ML fallback & Explain Mode")
