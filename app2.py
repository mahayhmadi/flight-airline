...
# Explain mode + under-chart "How to read / Use it to" notes + ML-aware What-If
...
# Default to ML-based (as requested). Design unchanged.
...
# Ensure 'stops' exists to avoid KeyError on apply
    ...
    st.markdown(
        ...
        <div class="soft-card">
          ...
          <ul style="margin:6px 0 0 16px">
            <li>KPIs show current revenue, optimized revenue, and the uplift.</li>
            <li>Table lists current vs recommended price per flight/cabin.</li>
            <li>Charts summarize where improvements come from.</li>
          </ul>
        </div>
        """, unsafe_allow_html=True
    )
...
# Safe defaults if dep_date is all NaT
...
if EXPLAIN_MODE:
    st.markdown(
        """
        <div class="soft-card">
          <b>How to read:</b> each row shows current price vs recommended price, with the revenue impact.
          <br><b>Use it to:</b> sort by uplift and prioritize changes with the biggest effect.
        </div>
        """, unsafe_allow_html=True
    )
...
try:
    ...
    # Avoid crashing if openpyxl is absent; we include it in requirements, but keep this safe.
...
with cB:
    ...
    if EXPLAIN_MODE:
        st.markdown(
            """
            <div class="soft-card">
              <b>How to read:</b> left = discounts, right = increases.
              <br><b>Use it to:</b> sanity-check that recommendations are balanced and realistic.
            </div>
            """, unsafe_allow_html=True
        )
...
with cC:
    ...
    if EXPLAIN_MODE:
        st.markdown(
            """
            <div class="soft-card">
              <b>How to read:</b> shows how recommended prices evolve as departure nears.
              <br><b>Use it to:</b> tune last-minute premiums or early-booking discounts.
            </div>
            """, unsafe_allow_html=True
        )
            ...
            if EXPLAIN_MODE:
                st.markdown(
                    ...
                    <div class="soft-card">
                      ...
                      <br><b>Use it to:</b> spot airline/season pockets where upside is highest.
                    </div>
                    """, unsafe_allow_html=True
                )
        else:
...
if {"optimal_revenue","current_expected_revenue","flight","airline"}.issubset(dff.columns):
    ...
    if EXPLAIN_MODE:
        st.markdown(
            """
            <div class="soft-card">
              <b>How to read:</b> the largest absolute revenue gains.
              <br><b>Use it to:</b> pick quick-win flights to action first.
            </div>
            """, unsafe_allow_html=True
        )
...
def load_ml_models():
    ...
    for cabin in ["Economy","Business","First"]:
        ...
        if os.path.exists(path):
            # scikit-learn must be installed to unpickle models
...
if engine_used=="ML-based" and not ml_models:
    st.warning("ML models not found. What-If will fallback to rule-based (include joblib models under models_seats_ml/).")
...
if selected != "— Select a flight —":
    ...
    if EXPLAIN_MODE:
        ...
        bullets = []
        if _demand >= 1.2: bullets.append("Demand is **high** → slight price lift is viable.")
        elif _demand <= 0.8: bullets.append("Demand is **low** → modest discounts help pickup.")
...
# =========================
st.markdown('<div class="hr-soft"></div>', unsafe_allow_html=True)
st.caption("© Dynamic Pricing Simulator — Streamlit + Plotly | ML-preferred loader with graceful Rule fallback | Explain Mode")
