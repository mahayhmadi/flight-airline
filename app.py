...
def load_csv_from_any(uploaded, url, local_hint, engine_choice):
    # 1) uploaded file
    ...
    if url.strip():
        try:
            return pd.read_csv(url.strip(), low_memory=False)
    ...
    if hint and os.path.exists(hint):
        return pd.read_csv(hint, low_memory=False)

    # 4) fallback by engine
    fallback = ML_CSV_REL if engine_choice == "ML-based" else DEFAULT_CSV_REL
    if os.path.exists(fallback):
        return pd.read_csv(fallback, low_memory=False)

    # Formal message for ML; informative for Rule-based handled in caller
    raise FileNotFoundError("CSV_NOT_FOUND")

try:
    df = load_csv_from_any(uploaded_file, csv_url, csv_path_hint, engine)
except FileNotFoundError:
    if engine == "ML-based":
        st.error("Please upload the CSV file.")
    else:
        st.error("Could not locate a CSV. Upload a file, paste a URL, or place the file next to app.py.")
    st.stop()
...
# DERIVATIONS & CHECKS
# =========================
# Ensure datetime columns if present
...
df["stops_norm"] = df["stops"].apply(normalize_stops) if "stops" in df.columns else np.nan
...
# MAIN FILTER BAR
# =========================
airlines_all = sorted(df["airline"].dropna().unique().tolist()) if "airline" in df.columns else []
...
with cfd:
    days_slider_max = int(max(1, df["days_left"].max())) if "days_left" in df.columns else 1
    f_daysmax = st.slider("Days left (max)", 1, days_slider_max, days_slider_max)

# Global mask
mask = pd.Series(True, index=df.index)
mask &= (df["dep_date"] >= date_from) & (df["dep_date"] <= date_to)
if sel_tod != "All" and "time_of_day" in df.columns:
    mask &= (df["time_of_day"] == sel_tod)
if sel_airlines and "airline" in df.columns:
    mask &= df["airline"].isin(sel_airlines)
if "stops_norm" in df.columns:
...
if f_class != "All" and "class" in df.columns:
    mask &= (df["class"] == f_class)
if f_season != "All" and "season_label" in df.columns:
    mask &= (df["season_label"] == f_season)
...
def expected_units_unified(engine_choice, models, row, test_price):
    if engine_choice=="ML-based" and models:
...
if selected != "— Select a flight —":
    ...
    if cap is not None and sold is not None:
        ...
        for _, r in sel.iterrows():
            ...
            new_rev_sim  += new_price * u_new   # <-- fixed newline issue
