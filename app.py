...
# Works with both Rule-based CSV and ML-based CSV outputs
...
ML_CSV      = r"C:\Users\bbuser\Desktop\flight_dashbord\final_optimized_flights_ML.csv"

st.sidebar.subheader("Data")
engine = st.sidebar.radio("Pricing engine", ["Rule-based", "ML-based"], index=0)
csv_default = ML_CSV if (engine=="ML-based" and os.path.exists(ML_CSV)) else DEFAULT_CSV
csv_path = st.sidebar.text_input("Optimized CSV path", value=csv_default)
...
def load_data(path: str) -> pd.DataFrame:
    return pd.read_csv(path, parse_dates=["departure_time","arrival_time"], infer_datetime_format=True, low_memory=False)
...
with c1:
    ...
    fig = go.Figure(data=[
        go.Bar(
            name="Before",
            x=class_rev["class"], y=class_rev["current_expected_revenue"], marker_color=BURGUNDY,
            hovertemplate="<b>%{x}</b><br>Before: OMR %{y:,.0f}<extra></extra>"
        ),
        go.Bar(
            name="After",
            x=class_rev["class"], y=class_rev["optimal_revenue"], marker_color="#9CA3AF",
    ...
    if EXPLAIN_MODE:
        st.caption("If the ‘After’ bar is higher than ‘Before’ for a cabin, the recommendation improved revenue there.")

with c2:
    st.markdown('<div class="section-title">% change vs current price — distribution</div>', unsafe_allow_html=True)
    fig = px.histogram(
        dff, x="pct_change_vs_current_price", nbins=40,
    ...
    if EXPLAIN_MODE:
        st.caption("Bars to the right are price increases; left are discounts. The balance should look reasonable.")
...
with c3:
    ...
    if EXPLAIN_MODE:
        st.caption("Closer to departure usually allows slightly higher prices; far out tends to be more price-sensitive.")
...
# Try to load ML models (only if engine=ML)
ml_models = None
if engine=="ML-based":
    try:
        ml_models = load_ml_models()
        if not ml_models:
            st.warning("ML models not found. What-If will fallback to rule-based. Run train_models.py.")
    except Exception:
        st.warning("Could not load ML models. What-If will fallback to rule-based.")
...
if selected != "— Select a flight —":
    ...
    if cap is not None and sold is not None:
        ...
        # Simulate using unified expected-units (same method for base and new)
        base_rev_sim = 0.0
        new_rev_sim  = 0.0
...
# =========================
st.markdown('<div class="hr-soft"></div>', unsafe_allow_html=True)
st.caption("© Dynamic Pricing Simulator — Muscat → London | Streamlit + Plotly | Clean English UI with Explain Mode")
